...
public class MyParticleEmitter {
	...
	public boolean tinted=true,autotrans=false;
	...
	public float newmvalue=1, newMvalue=1;
	public float newvvalue=1, newVvalue=1;
	...
	public MyParticleEmitter (boolean tinted) {
		this.tinted=tinted;
		initialize();
	}

	public MyParticleEmitter (BufferedReader reader) throws IOException {
		initialize();
		load(reader);
	}
	
	public MyParticleEmitter (BufferedReader reader, boolean tinted) throws IOException {
		this.tinted=tinted;
		initialize();
		load(reader);
	}

	public MyParticleEmitter (MyParticleEmitter emitter) {
		...
		if (tinted)tintValue.load(emitter.tintValue);
	...
	public MyParticleEmitter (MyParticleEmitter emitter, boolean tinted) {
		this.tinted=tinted;
		sprite = emitter.sprite;
		name = emitter.name;
		imagePath = emitter.imagePath;
	...
	public void update (float delta, float lowtransparency, float hitransparency, float lowvelocity, float hivelocity,boolean speedCtrl) {
		this.vCtrl=speedCtrl;
		this.newmvalue= lowtransparency;
		this.newMvalue=hitransparency;
		this.newvvalue= lowvelocity;
		this.newVvalue=hivelocity;
		accumulator += delta * 1000;
		if (accumulator < 1) return;
		int deltaMillis = (int)accumulator;
		accumulator -= deltaMillis;
...
*/
	...
	public void drawext (Batch batch) {
		Particle[] particles = this.particles;
		boolean[] active = this.active;

		for (int i = 0, n = active.length; i < n; i++)
			if (active[i] && onScreen(particles[i])) particles[i].draw(batch);
	}
	...
	public void draw (Batch batch, float delta, float ratioxy) {
		accumulator += delta * 1000;
		if (accumulator < 1) {
			draw(batch);
			return;
		...
		for (int i = 0, n = active.length; i < n; i++) {
			if (active[i]) {
				Particle particle = particles[i];
				if (updateParticle(particle, delta, deltaMillis, ratioxy)){
					particle.draw(batch);
			}
				else {
					active[i] = false;
	...
	private void restart () {
		...
		if (tinted && tintValue.timeline.length > 1) updateFlags |= UPDATE_TINT;
	...
	private void activateParticle (int index) {
		...
		if(!autotrans){
			transparencyValue.setLow(newmvalue);
			transparencyValue.setHigh(newMvalue);
			particle.transparency = transparencyValue.newLowValue();
			particle.transparencyDiff = newMvalue - particle.transparency;
		}
		
		if (velocityValue.active) {
			if(vCtrl==false){
			velocityValue.setrandomLow(newvvalue, newVvalue);
			velocityValue.setrandomHigh(newvvalue,newVvalue);
			}
			if(vCtrl==true){
				velocityValue.setLow(newvvalue, newVvalue);
				velocityValue.setHigh(newvvalue,newVvalue);
				}
	...
	private boolean updateParticle (Particle particle, float delta, int deltaMillis) {
		...
		if(tinted){
			...
			particle.setColor(sprite.getColor().r, sprite.getColor().g, sprite.getColor().b,
			particle.transparency + particle.transparencyDiff * transparencyValue.getScale(percent));
		}
	...
	private boolean updateParticle (Particle particle, float delta, int deltaMillis, float ratioxy) {
		int life = particle.currentLife - deltaMillis;
		if (life <= 0) return false;
		particle.currentLife = life;
		...
		if ((updateFlags & UPDATE_VELOCITY) != 0) {
			...
			if ((updateFlags & UPDATE_ANGLE) != 0) {
				float angle = particle.angle + particle.angleDiff * angleValue.getScale(percent);
				velocityX = velocity * MathUtils.cosDeg(angle);
				velocityY = velocity*ratioxy * MathUtils.sinDeg(angle);
				if ((updateFlags & UPDATE_ROTATION) != 0) {
					float rotation = particle.rotation + particle.rotationDiff * rotationValue.getScale(percent);
					if (aligned) rotation += angle;
					particle.setRotation(rotation);
				...
				velocityX = velocity * particle.angleCos;
				velocityY = velocity*ratioxy * particle.angleSin;
				if (aligned || (updateFlags & UPDATE_ROTATION) != 0) {
					float rotation = particle.rotation + particle.rotationDiff * rotationValue.getScale(percent);
					if (aligned) rotation += particle.angle;
					particle.setRotation(rotation);
	...
	static public class ScaledNumericValue extends RangedNumericValue {
		...
		public float setrandomHigh(float newvvalue, float newVvalue) {
			return  newvvalue + (newVvalue - newvvalue) * MathUtils.random();
			
		}

		public float setrandomLow(float newvvalue, float newVvalue) {
			return newvvalue + (newVvalue - newvvalue) * MathUtils.random();
