...
public class Tuto0 implements Screen {
	...
	private Music music0, musicspace;
...
//		asteroid2bg = new RandomBg(asteroid2, 3, 3, 149, 181, 0.25f, 0.03f, camera, 140);
		...
		music0 = Assets.manager.get(Assets.music,Music.class);
	...
	public void render(float delta) {
	   if (tutocheck2 && !tutoover && mole.vitesse < 12F){
            Mole mole1 = mole;
            mole1.vitesse = mole1.vitesse + 0.07F;
        } 
	   else if (!tutocheck2 && !tutoover){
            mole.vitesse = 4.7F;
       }
       if (mole.timeslow && mole.timetimer > 0.0F){
           if (timeScale > 0.4F) {
                timeScale = timeScale * 0.97F;
           } 
           else{
                timeScale = 0.4F;
           }
       } 
       else if (!tutoover && tutocheck2 && !mole.isudg0 && mole.molebody.getLinearVelocity().y < 0.0F){
    	   
           if (timeScale > 0.36F) {
               timeScale = timeScale * 0.9F;
           } 
           else{
               timeScale = 0.36F;
           }
...
//		}
		...
		if (!tutoover)
        {
            if (tuto7.getColor().a == 1.0F)
            {
                float f1 = 3.45F * 1.0F;
                float f3 = mole.molebody.getPosition().x;
                float f5 = 3.45F / 2.0F;
                float f7 = mole.molebody.getPosition().y;
                float f9 = f1 / 2.0F;
                float  f11 = MathUtils.cosDeg(45F);
                float f13 = MathUtils.sinDeg(45F);
                float f15 = MathUtils.sinDeg(45F);
                float  f17 = -MathUtils.cosDeg(45F);
                arrow1.setSize(3.45F, f1);
                arrow1.setPosition((f3 - f5 - f11 * -2.3F) + f15 * 0.0F, (f7 - f9 - f13 * -2.3F) + f17 * 0.0F);
                arrow1.setOriginCenter();
                arrow1.setRotation(45F);
                arrow1.draw(batch);
            } else
            if (tuto8.getColor().a == 1.0F)
            {
                float f2 = 3.45F * 1.0F;
                float f4 = mole.molebody.getPosition().x;
                float f6 = 3.45F / 2.0F;
                float f8 = mole.molebody.getPosition().y;
                float f10 = f2 / 2.0F;
                float f12 = MathUtils.cosDeg(mole.molebody.getLinearVelocity().angle());
                float f14 = MathUtils.sinDeg(mole.molebody.getLinearVelocity().angle());
                float f16 = MathUtils.sinDeg(mole.molebody.getLinearVelocity().angle());
                float f18 = -MathUtils.cosDeg(mole.molebody.getLinearVelocity().angle());
                arrow1.setSize(3.45F, f2);
                arrow1.setPosition((f4 - f6 - f12 * -2.3F) + f16 * 0.0F, (f8 - f10 - f14 * -2.3F) + f18 * 0.0F);
                arrow1.setOriginCenter();
                arrow1.setRotation(mole.molebody.getLinearVelocity().angle());
                arrow1.draw(batch);
                f2 = 3.45F * 1.0F;
                f4 = mole.molebody.getPosition().x;
                f6 = 3.45F / 2.0F;
                f8 = mole.molebody.getPosition().y;
                f10 = f2 / 2.0F;
                f12 = MathUtils.cosDeg(mole.molebody.getAngle() * 57.29578F);
                f14 = MathUtils.sinDeg(mole.molebody.getAngle() * 57.29578F);
                f16 = MathUtils.sinDeg(mole.molebody.getAngle() * 57.29578F);
                f18 = -MathUtils.cosDeg(mole.molebody.getAngle() * 57.29578F);
                arrow2.setSize(3.45F, f2);
                arrow2.setPosition((f4 - f6 - f12 * -2.3F) + f16 * 0.0F, (f8 - f10 - f14 * -2.3F) + f18 * 0.0F);
                arrow2.setOriginCenter();
                arrow2.setRotation(mole.molebody.getAngle() * 57.29578F);
                if (mole.anglediff < mole.landangle / 2.0F && mole.anglediff > -mole.landangle / 2.0F)
                {
                    arrow2.setColor(0.0F, 1.0F, 0.0F, 1.0F);
                } else
                if (mole.anglediff < mole.landangle && mole.anglediff > -mole.landangle)
                {
                    arrow2.setColor(1.0F, 1.0F, 0.0F, 1.0F);
                } else
                {
                    arrow2.setColor(1.0F, 0.0F, 0.0F, 1.0F);
                }
                arrow2.draw(batch);
                if (mole.molebody.getLinearVelocity().angle() < mole.angle || mole.angle - mole.molebody.getLinearVelocity().angle() < -180F)
                {
                    turnright.addAction(alpha(1.0F, 0.05F, Interpolation.fade));
                    turnleft.addAction(alpha(0.0F, 0.01F, Interpolation.fade));
                } else
                {
                    turnright.addAction(alpha(0.0F, 0.05F, Interpolation.fade));
                    turnleft.addAction(alpha(1.0F, 0.05F, Interpolation.fade));
                }
            }
        }
		...
		//////////////===================  MUSIC  =====================///////////////////
		if(mole.molebody.getPosition().y>50) {
			music0.setVolume(MathUtils.clamp(mole.vitesse/40, 0.2f, 5f)/(mole.molebody.getPosition().y-50));
		}
		else
			music0.setVolume(MathUtils.clamp(mole.vitesse/40, 0.2f, 0.5f));
       ...
       //////////////============================  UI  =================================//////////
		...
		if (!tutoover) {
            if (in.leftK && (mole.molebody.getAngle() * 360F) / 6.28F > 60F && !tutocheck1)
            {
                tutocheck1 = true;
                tuto4.addAction(sequence(delay(1.0F), alpha(0.0F, 1.0F)));
                tuto5.addAction(sequence(delay(2.0F), alpha(1.0F, 1.0F)));
                turnleft.clearActions();
                turnleft.addAction(sequence(delay(1.0F), alpha(0.0F, 1.0F)));
                turnright.addAction(sequence(delay(2.0F), forever(sequence(alpha(1.0F, 1.0F, Interpolation.sine), alpha(0.0F, 1.0F, Interpolation.sine)))));
            }
            if (tuto5.getColor().a == 1.0F && mole.molebody.getAngle() < 0.0F && !tutocheck2)
            {
                tutocheck2 = true;
                in.rightK = false;
                tuto5.addAction(sequence(delay(1.0F), alpha(0.0F, 1.0F)));
                tuto6.addAction(sequence(delay(2.0F), alpha(1.0F, 1.0F)));
                turnright.clearActions();
                turnright.addAction(sequence(delay(1.0F), alpha(0.0F, 1.0F)));
                mole.autotuto = true;
                tutobg.setTouchable(Touchable.enabled);
                c2c.addAction(sequence(delay(1.0F), com.badlogic.gdx.scenes.scene2d.actions.Actions.show()));
            }

            if (tuto7.getColor().a == 1.0F)
            {
                if (mole.angle < 45F || mole.angle > 180F)
                {
                    turnright.addAction(alpha(0.0F, 0.05F, Interpolation.fade));
                    turnleft.addAction(alpha(1.0F, 0.05F, Interpolation.fade));
                } else
                {
                    turnright.addAction(alpha(1.0F, 0.05F, Interpolation.fade));
                    turnleft.addAction(alpha(0.0F, 0.05F, Interpolation.fade));
                }
            }
            if (tuto7.getColor().a == 1.0F && timeScale < 1.0F)
            {
                tuto7.addAction(sequence(delay(0.1F), alpha(0.0F, 0.2F)));
                tuto8.addAction(sequence(delay(0.2F), alpha(1.0F, 0.2F)));
                molegl = mole.goodland;
            }
            if (tuto8.getColor().a == 1.0F && timeScale == 1.0F && mole.goodland > molegl)
            {
                tuto8.addAction(sequence(delay(0.0F), alpha(0.0F, 1.0F)));
                tuto9.addAction(sequence(delay(1.0F), alpha(1.0F, 0.2F)));
                turnright.addAction(alpha(0.0F, 0.05F, Interpolation.fade));
                turnleft.addAction(alpha(0.0F, 0.05F, Interpolation.fade));
            }
            if (tuto9.getColor().a == 1.0F)
            {
                if (mole.angle < 45F || mole.angle > 180F)
                {
                    turnright.addAction(alpha(0.0F, 0.05F, Interpolation.fade));
            ...
            if (tuto9.getColor().a == 1.0F && timeScale < 1.0F)
            {
                tuto9.addAction(sequence(delay(0.1F), alpha(0.0F, 0.2F)));
                tuto10.addAction(sequence(delay(0.2F), alpha(1.0F, 0.2F), delay(0.6F), alpha(0.0F, 0.1F)));
                tuto11.addAction(sequence(delay(1.1F), alpha(1.0F, 0.2F)));
                molegl = mole.goodland;
            }
            if (tuto11.getColor().a == 1.0F || tuto10.getColor().a == 1.0F)
            {
                if (mole.molebody.getLinearVelocity().angle() < mole.angle || mole.angle - mole.molebody.getLinearVelocity().angle() < -180F)
                {
                    turnright.addAction(alpha(1.0F, 0.05F, Interpolation.fade));
            ...
            if (tuto11.getColor().a == 1.0F && timeScale == 1.0F && mole.goodland > molegl)
            {
                tuto11.addAction(sequence(delay(0.0F), alpha(0.0F, 1.0F)));
                tuto12.addAction(sequence(delay(1.0F), alpha(1.0F, 0.2F)));
                turnright.addAction(alpha(0.0F, 0.05F, Interpolation.fade));
                turnleft.addAction(alpha(0.0F, 0.05F, Interpolation.fade));
                tutobg.setTouchable(Touchable.enabled);
                c2c.addAction(sequence(delay(1.0F), com.badlogic.gdx.scenes.scene2d.actions.Actions.show()));
